...
class CustomReceiverJnetPcap(var device:String)
  extends Receiver[PacketInfo](StorageLevel.MEMORY_AND_DISK_2) with Logging  {
  def main() {
    println("In main")
  }
      ...
      def nextPacket(packet: PcapPacket, user: String) {
        if (packet.hasHeader(ip)) {
         ...
         // store(org.jnetpcap.packet.format.FormatUtils.ip(dIP)+","+packet.getCaptureHeader.wirelen().toString)
          store(new PacketInfo(org.jnetpcap.packet.format.FormatUtils.ip(dIP), packet.getCaptureHeader.wirelen()))
